...
async function openRoutineEditor(routineId, mc) {
 ...
 `;
  ...
  const _reNorm = s => String(s).toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g,'');
  const _RE_EN_ES = [
  ...
  const _reExMatch = (ex, q) => {
    const en = _reNorm(ex.n), em = _reNorm(ex.m);
    if (en.includes(q) || em.includes(q)) return true;
    for (const [eng, esp] of _RE_EN_ES) {
      const espN = _reNorm(esp);
  ...
  searchEl.addEventListener('input', () => {
    const q = _reNorm(searchEl.value.trim());
    if (!q) { resultsEl.style.display='none'; return; }
    const hits = EXERCISES.filter(e => _reExMatch(e, q)).slice(0,20);
